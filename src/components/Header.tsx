  ...
  return (
    <div className={`flex items-center gap-1 text-xs font-semibold ${className}`}>
      <Link
        href={pathname}
        locale="kk"
        className={`px-2 py-1 rounded transition-colors ${locale === "kk" ? "bg-gold text-forest" : "text-cream/70 hover:text-gold"}`}
      >
        ҚАЗ
      </Link>
      ...
      <Link
        href={pathname}
        locale="ru"
        className={`px-2 py-1 rounded transition-colors ${locale === "ru" ? "bg-gold text-forest" : "text-cream/70 hover:text-gold"}`}
      >
        РУС
      </Link>
    </div>
...
export default function Header() {
  ...
  return (
    <header className="bg-forest text-cream shadow-lg sticky top-0 z-50">
        ...
        <div className="flex items-center justify-between h-14 sm:h-16 lg:h-20">
            ...
            {t.nav.map((link) => (
              <Link
                ...
                className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                  pathname === link.href
                    ? "bg-gold text-forest"
                    : "text-cream/90 hover:bg-forest-light hover:text-gold"
                }`}
              >
                {link.label}
              </Link>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="lg:hidden p-2 -mr-2 rounded-md text-cream hover:bg-forest-light transition-colors"
      ...
      {/* Mobile menu overlay */}
      <div
        className={`lg:hidden fixed inset-0 top-14 sm:top-16 bg-forest z-40 transition-transform duration-300 ease-in-out ${
          ...
          {t.nav.map((link) => (
            <Link
              ...
              className={`block px-4 py-3 rounded-lg text-lg font-medium transition-colors ${
                pathname === link.href
                  ? "bg-gold text-forest"
                  : "text-cream/90 active:bg-forest-light"
              }`}
            >
              {link.label}
            </Link>
